...
import ru.tinkoff.kora.s3.client.exception.*;
import ru.tinkoff.kora.s3.client.impl.xml.*;
...
import ru.tinkoff.kora.s3.client.model.response.ListMultipartUploadsResult;
import ru.tinkoff.kora.s3.client.model.response.ListMultipartUploadsResult.Upload;
import ru.tinkoff.kora.s3.client.model.response.ListPartsResult;
                    ...
                    try (var rs = this.httpClient.execute(request);
                        ...
                        if (rs.code() == 200) {
                            try (var is = body.asInputStream()) {
                                ...
                                var listResult = ru.tinkoff.kora.s3.client.impl.xml.ListBucketResult.fromXml(new ByteArrayInputStream(bytes));
        ...
        return Observation.scoped(observation)
            .call(() -> {
                ...
                try (var rs = this.httpClient.execute(request)) {
                    ...
                    observation.observeAwsExtendedId(rs.headers().getFirst("x-amz-id-2"));
                    if (rs.code() == HttpURLConnection.HTTP_OK) {
                        var xml = ru.tinkoff.kora.s3.client.impl.xml.ListMultipartUploadsResult.fromXml(rs.body().asInputStream());
        ...
        return Observation.scoped(observation)
            .call(() -> {
                ...
                try (var rs = this.httpClient.execute(request);
                    ...
                    observation.observeAwsExtendedId(rs.headers().getFirst("x-amz-id-2"));
                    if (rs.code() == HttpURLConnection.HTTP_OK) {
                        var listPartsResult = ru.tinkoff.kora.s3.client.impl.xml.ListPartsResult.fromXml(body.asInputStream());
