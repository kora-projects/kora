package io.koraframework.s3.client.impl;
...
import io.koraframework.s3.client.exception.*;
import io.koraframework.s3.client.impl.xml.*;
...
import io.koraframework.s3.client.model.response.ListMultipartUploadsResult;
import io.koraframework.s3.client.model.response.ListMultipartUploadsResult.Upload;
import io.koraframework.s3.client.model.response.ListPartsResult;
import io.koraframework.s3.client.telemetry.S3ClientTelemetry;
                    ...
                    try (var rs = this.httpClient.execute(request);
                        ...
                        if (rs.code() == 200) {
                            try (var is = body.asInputStream()) {
                                ...
                                var listResult = io.koraframework.s3.client.impl.xml.ListBucketResult.fromXml(new ByteArrayInputStream(bytes));
        ...
        return Observation.scoped(observation)
            .call(() -> {
                ...
                try (var rs = this.httpClient.execute(request)) {
                    ...
                    observation.observeAwsExtendedId(rs.headers().getFirst("x-amz-id-2"));
                    if (rs.code() == HttpURLConnection.HTTP_OK) {
                        var xml = io.koraframework.s3.client.impl.xml.ListMultipartUploadsResult.fromXml(rs.body().asInputStream());
        ...
        return Observation.scoped(observation)
            .call(() -> {
                ...
                try (var rs = this.httpClient.execute(request);
                    ...
                    observation.observeAwsExtendedId(rs.headers().getFirst("x-amz-id-2"));
                    if (rs.code() == HttpURLConnection.HTTP_OK) {
                        var listPartsResult = io.koraframework.s3.client.impl.xml.ListPartsResult.fromXml(body.asInputStream());
