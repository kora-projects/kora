...
public class DefaultS3ClientTelemetry implements S3ClientTelemetry {
    ...
    protected final ConcurrentMap<Tags, Timer> requestDurationCache = new ConcurrentHashMap<>();
    ...
    public DefaultS3ClientTelemetry(S3ClientTelemetryConfig config,
        ...
        this.requestDurationMeter = tags ->
            requestDurationCache.computeIfAbsent(Tags.of(tags), _ -> createMetricClientDuration()
                .tags((Iterable<Tag>) tags)
                .register(this.meterRegistry));

        var logger = LoggerFactory.getLogger(S3ClientTelemetry.class);
    ...
    public S3ClientObservation observe(String operation, String bucket) {
        ...
        logger.debug("KoraS3Client starting S3 operation '{}' on bucket: {}", operation, bucket);
        return new DefaultS3ClientObservation(bucket, operation, config, span, logger, requestDurationMeter);
    }

    protected Timer.Builder createMetricClientDuration() {
        var staticTags = new ArrayList<Tag>(1 + this.config.metrics().tags().size());
        staticTags.add(Tag.of(RpcIncubatingAttributes.RPC_SYSTEM.getKey(), "s3-kora"));
