...
final class KnownSizeAwsChunkedHttpBody implements HttpBodyOutput {
    ...
    public KnownSizeAwsChunkedHttpBody(S3RequestSigner signer, String region, int uploadChunkSize, String contentType, String rqSignature, S3Client.ContentWriter contentWriter, @Nullable String sha256Base64) {
        this.region = region;
        this.signer = signer;
    ...
    private String processChunk(String previousSignature, byte[] buf, int read, OutputStream os) throws IOException {
        var amzDate = this.date.format(S3RequestSigner.AMZ_DATE_FORMAT);
        var signerDate = this.date.format(S3RequestSigner.SIGNER_DATE_FORMAT);
        ...
        os.write(S3RequestSigner.CLRF_BYTES);
        os.write(buf, 0, read);
        os.write(S3RequestSigner.CLRF_BYTES);
