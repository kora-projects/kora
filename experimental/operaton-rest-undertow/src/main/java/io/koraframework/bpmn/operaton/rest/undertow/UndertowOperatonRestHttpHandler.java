...
import io.koraframework.bpmn.operaton.rest.undertow.UndertowPathMatcher.HttpMethodPath;
...
final class UndertowOperatonRestHttpHandler implements Lifecycle, Wrapped<HttpHandler> {

    private static final Logger logger = LoggerFactory.getLogger(UndertowOperatonRestHttpHandler.class);
    ...
    private final OperatonRestConfig restConfig;
    private final OperatonRestTelemetry telemetry;

    private volatile DeploymentManager deploymentManager;
    private volatile HttpHandler realhttpHandler;

    UndertowOperatonRestHttpHandler(Iterable<Application> applications,
                                    OperatonRestConfig restConfig,
                                    OperatonRestTelemetry telemetry) {
    ...
    public HttpHandler value() {
        if (restConfig.cors().enabled()) {
            return new UndertowCorsFilter(realhttpHandler, restConfig);
        } else {
    ...
    public void init() throws Exception {
        logger.debug("Operaton Rest Handler (Undertow) configuring...");
        ...
        var classLoader = UndertowOperatonRestHttpHandler.class.getClassLoader();
        di.setClassLoader(classLoader);
        di.setContextPath(restConfig.path());
        di.setDeploymentName("ResteasyOperatonKora");
        deploymentManager = container.addDeployment(di);
        deploymentManager.deploy();

        var restPaths = getRestPaths(restConfig);
        var restMatcher = new UndertowPathMatcher(restPaths);

        var restHandler = deploymentManager.start();
        root.addPrefixPath(restConfig.path(), exchange -> {
        ...
        root.addPrefixPath("/", new OpenApiHttpHandler(restConfig));
        this.realhttpHandler = new VirtualThreadHttpHandler("operaton-rest", root);

        logger.info("Operaton Rest Handler (Undertow) configured in {}", TimeUtils.tookForLogging(started));
    }

    @Override
    public void release() throws Exception {
        logger.debug("Operaton Rest Handler (Undertow) stopping...");
        final long started = TimeUtils.started();

        deploymentManager.stop();

        logger.info("Operaton Rest Handler (Undertow) stopped in {}", TimeUtils.tookForLogging(started));
    }

    private static List<HttpMethodPath> getRestPaths(OperatonRestConfig restConfig) {
