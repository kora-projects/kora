...
public final class Opentelemetry123HttpServerMetrics implements HttpServerMetrics {
    ...
    /**
     * @see #Opentelemetry123HttpServerMetrics(MeterRegistry, MicrometerHttpServerTagsProvider, TelemetryConfig.MetricsConfig, HttpServerConfig)
     */
    @Deprecated
    public Opentelemetry123HttpServerMetrics(MeterRegistry meterRegistry,
    ...
    public void requestFinished(int statusCode, HttpResultCode resultCode, String scheme, String host, String method, String pathTemplate, HttpHeaders headers, long processingTimeNanos, Throwable exception) {
        var countKey = new ActiveRequestsKey(method, pathTemplate, host, port, scheme);
        var counter = requestCounters.computeIfAbsent(countKey, activeRequestsKey -> {
            var c = new AtomicInteger(0);
            this.registerActiveRequestsGauge(activeRequestsKey, c);
            return c;
        });
    ...
    private DistributionSummary requestDuration(DurationKey key) {
        var builder = DistributionSummary.builder("http.server.request.duration")
            .serviceLevelObjectives(this.metricsConfig.slo(TelemetryConfig.MetricsConfig.OpentelemetrySpec.V123))
            .baseUnit("s")
