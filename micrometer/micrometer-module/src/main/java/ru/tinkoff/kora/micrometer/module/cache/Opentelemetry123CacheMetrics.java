...
public final class Opentelemetry123CacheMetrics implements CacheMetrics {
    ...
    public Opentelemetry123CacheMetrics(MeterRegistry meterRegistry, TelemetryConfig.MetricsConfig config) {
    ...
    public void recordSuccess(@Nonnull CacheTelemetryOperation operation, long durationInNanos, @Nullable Object valueFromCache) {
        final DurationKey key = new DurationKey(operation.cacheName(), operation.origin(), operation.name(), STATUS_SUCCESS);
        durations.computeIfAbsent(key, k -> duration(key, null))
            .record((double) durationInNanos / 1_000_000_000);
