...
public class LettuceStandaloneCacheClient implements RedisCacheClient, Lifecycle {
    ...
    protected BoundedAsyncPool<StatefulRedisConnection<byte[], byte[]>> pool;
    protected StatefulRedisConnection<byte[], byte[]> connection;

    // always use async cause sync uses JDK Proxy wrapped async impl
    protected RedisAsyncCommands<byte[], byte[]> commands;

    public LettuceStandaloneCacheClient(RedisClient redisClient, RedisURI redisURI) {
    ...
    public void flushAll() {
        commands.flushall(FlushMode.SYNC).toCompletableFuture().join();
    }
    ...
    public void init() {
        try {
            ...
            throw new IllegalStateException("Redis client (Lettuce) failed to start in standalone mode: %s; check Redis address, credentials, TLS settings, and network access".formatted(e.getMessage()), e);
        }
    }
