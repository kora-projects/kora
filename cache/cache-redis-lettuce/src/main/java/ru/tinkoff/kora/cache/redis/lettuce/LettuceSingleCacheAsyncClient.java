...
final class LettuceSingleCacheAsyncClient implements RedisCacheAsyncClient, Lifecycle {

    private static final Logger logger = LoggerFactory.getLogger(LettuceSingleCacheAsyncClient.class);
    ...
    private BoundedAsyncPool<StatefulRedisConnection<byte[], byte[]>> lettucePool;

    LettuceSingleCacheAsyncClient(RedisClient lettuceClient,
    ...
    public CompletionStage<Void> psetex(Map<byte[], byte[]> keyAndValue, long expireAfterMillis) {
        return lettucePool.acquire().thenCompose(connection -> {
            ...
            return CompletableFuture.allOf(futures.toArray(CompletableFuture[]::new))
                .whenComplete((s, throwable) -> lettucePool.release(connection));
        });
    }
    ...
    public void init() {
        ...
        this.lettucePool = AsyncConnectionPoolSupport.createBoundedObjectPool(() -> lettuceClient.connectAsync(ByteArrayCodec.INSTANCE, redisURI), poolConfig);
