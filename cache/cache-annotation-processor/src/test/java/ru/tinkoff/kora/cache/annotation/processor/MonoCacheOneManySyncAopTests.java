...
class MonoCacheOneManySyncAopTests implements CaffeineCacheModule, RedisCacheModule {

    private static final String CACHED_IMPL_1 = "ru.tinkoff.kora.cache.annotation.processor.testcache.$DummyCache11Impl";
    private static final String CACHED_IMPL_2 = "ru.tinkoff.kora.cache.annotation.processor.testcache.$DummyCache13Impl";
    private static final String CACHED_SERVICE = "ru.tinkoff.kora.cache.annotation.processor.testdata.reactive.mono.$CacheableMonoOneManySync__AopProxy";

    private DummyCache11 cache1 = null;
    private DummyCache13 cache2 = null;
    private CacheableMonoOneManySync service = null;

    private CacheableMonoOneManySync getService() {
        if (service != null) {
            return service;
        }

        try {
            var classLoader = TestUtils.annotationProcess(List.of(DummyCache11.class, DummyCache13.class, CacheableMonoOneManySync.class),
            ...
            service = (CacheableMonoOneManySync) serviceConstructor.newInstance(cache1, cache2);
            return service;
        } catch (Exception e) {
    ...
    void getFromCacheLevel2AndThenSaveCacheLevel1() {
        ...
        // when
        final String valueFromLevel2 = service.getValue("1").block(Duration.ofMinutes(1));
        service.value = "2";

        // then
        final String valueFromLevel1 = service.getValue("1").block(Duration.ofMinutes(1));
