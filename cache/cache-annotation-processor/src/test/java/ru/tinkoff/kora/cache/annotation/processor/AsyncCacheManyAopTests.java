...
class AsyncCacheManyAopTests implements CaffeineCacheModule, RedisCacheModule {
    ...
    private static final String CACHED_SERVICE = "ru.tinkoff.kora.cache.annotation.processor.testdata.async.$CacheableAsyncMany__AopProxy";

    private DummyCache21 cache1 = null;
    private DummyCache22 cache2 = null;
    private CacheableAsyncMany service = null;

    private CacheableAsyncMany getService() {
        if (service != null) {
            return service;
        }

        try {
            var classLoader = TestUtils.annotationProcess(List.of(DummyCache21.class, DummyCache22.class, CacheableAsyncMany.class),
            ...
            service = (CacheableAsyncMany) serviceConstructor.newInstance(cache1, cache2);
            return service;
        } catch (Exception e) {
    ...
    void getFromCacheWhenWasCacheEmpty() {
        ...
        // when
        final String notCached = service.getValue("1", BigDecimal.ZERO).toCompletableFuture().join();
        service.value = "2";

        // then
        final String fromCache = service.getValue("1", BigDecimal.ZERO).toCompletableFuture().join();
    ...
    void getFromCacheLevel2AndThenSaveCacheLevel1() {
        ...
        // when
        final String valueFromLevel2 = service.getValue("1", BigDecimal.ZERO).toCompletableFuture().join();
        service.value = "2";

        // then
        final String valueFromLevel1 = service.getValue("1", BigDecimal.ZERO).toCompletableFuture().join();
    ...
    void getFromCacheWhenCacheFilled() {
        ...
        // when
        final String initial = service.getValue("1", BigDecimal.ZERO).toCompletableFuture().join();
    ...
    void getFromCacheWrongKeyWhenCacheFilled() {
        ...
        // then
        final String fromCache = service.getValue("2", BigDecimal.ZERO).toCompletableFuture().join();
    ...
    void getFromCacheWhenCacheFilledOtherKey() {
        ...
        final String cached = service.putValue(BigDecimal.ZERO, "5", "1").toCompletableFuture().join();
        service.value = "2";
        final String initial = service.getValue("2", BigDecimal.ZERO).toCompletableFuture().join();
