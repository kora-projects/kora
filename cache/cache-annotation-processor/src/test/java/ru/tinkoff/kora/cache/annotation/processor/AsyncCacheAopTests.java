...
class AsyncCacheAopTests implements CaffeineCacheModule {

    private static final String CACHED_IMPL = "ru.tinkoff.kora.cache.annotation.processor.testcache.$DummyCache21Impl";
    private static final String CACHED_SERVICE = "ru.tinkoff.kora.cache.annotation.processor.testdata.async.$CacheableAsync__AopProxy";

    private DummyCache21 cache = null;
    private CacheableAsync service = null;

    private CacheableAsync getService() {
        if (service != null) {
            return service;
        }

        try {
            var classLoader = TestUtils.annotationProcess(List.of(DummyCache21.class, CacheableAsync.class),
            ...
            service = (CacheableAsync) serviceConstructor.newInstance(cache);
            return service;
        } catch (Exception e) {
    ...
    void getFromCacheWhenCacheInvalidate() {
        ...
        final String cached2 = service.putValue(BigDecimal.ZERO, "5", "2").toCompletableFuture().join();
