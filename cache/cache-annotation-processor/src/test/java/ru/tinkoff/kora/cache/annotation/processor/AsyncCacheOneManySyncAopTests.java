...
class AsyncCacheOneManySyncAopTests implements CaffeineCacheModule, RedisCacheModule {
    ...
    private static final String CACHED_SERVICE = "ru.tinkoff.kora.cache.annotation.processor.testdata.async.$CacheableAsyncOneManySync__AopProxy";

    private DummyCache11 cache1 = null;
    private DummyCache13 cache2 = null;
    private CacheableAsyncOneManySync service = null;

    private CacheableAsyncOneManySync getService() {
        if (service != null) {
            return service;
        }

        try {
            var classLoader = TestUtils.annotationProcess(List.of(DummyCache11.class, DummyCache13.class, CacheableAsyncOneManySync.class),
            ...
            service = (CacheableAsyncOneManySync) serviceConstructor.newInstance(cache1, cache2);
            return service;
        } catch (Exception e) {
    ...
    void getFromCacheLevel2AndThenSaveCacheLevel1() {
        ...
        // when
        final String valueFromLevel2 = service.getValue("1").toCompletableFuture().join();
        service.value = "2";

        // then
        final String valueFromLevel1 = service.getValue("1").toCompletableFuture().join();
