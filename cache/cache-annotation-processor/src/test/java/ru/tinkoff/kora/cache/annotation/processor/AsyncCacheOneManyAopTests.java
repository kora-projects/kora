...
class AsyncCacheOneManyAopTests implements CaffeineCacheModule, RedisCacheModule {
    ...
    private static final String CACHED_SERVICE = "ru.tinkoff.kora.cache.annotation.processor.testdata.async.$CacheableAsyncOneMany__AopProxy";

    private DummyCache11 cache1 = null;
    private DummyCache12 cache2 = null;
    private CacheableAsyncOneMany service = null;

    private CacheableAsyncOneMany getService() {
        if (service != null) {
            return service;
        }

        try {
            var classLoader = TestUtils.annotationProcess(List.of(DummyCache11.class, DummyCache12.class, CacheableAsyncOneMany.class),
            ...
            service = (CacheableAsyncOneMany) serviceConstructor.newInstance(cache1, cache2);
            return service;
        } catch (Exception e) {
