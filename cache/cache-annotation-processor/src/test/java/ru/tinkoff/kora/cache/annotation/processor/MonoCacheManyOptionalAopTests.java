...
class MonoCacheManyOptionalAopTests implements CaffeineCacheModule, RedisCacheModule {
    ...
    private static final String CACHED_SERVICE = "ru.tinkoff.kora.cache.annotation.processor.testdata.reactive.mono.$CacheableMonoManyOptional__AopProxy";

    private DummyCache21 cache1 = null;
    private DummyCache22 cache2 = null;
    private CacheableMonoManyOptional service = null;

    private CacheableMonoManyOptional getService() {
        if (service != null) {
            return service;
        }

        try {
            var classLoader = TestUtils.annotationProcess(List.of(DummyCache21.class, DummyCache22.class, CacheableMonoManyOptional.class),
            ...
            service = (CacheableMonoManyOptional) serviceConstructor.newInstance(cache1, cache2);
            return service;
        } catch (Exception e) {
    ...
    void getFromCacheLevel2AndThenSaveCacheLevel1() {
        ...
        // when
        final String valueFromLevel2 = service.getValue("1", BigDecimal.ZERO).block(Duration.ofMinutes(1)).orElseThrow();
        service.value = "2";

        // then
        final String valueFromLevel1 = service.getValue("1", BigDecimal.ZERO).block(Duration.ofMinutes(1)).orElseThrow();
    ...
    void getFromCacheWhenCacheInvalidate() {
        ...
        assertEquals(initial, cached);
        service.value = "2";
        service.evictValue("1", BigDecimal.ZERO).block(Duration.ofMinutes(1)).orElseThrow();
    ...
    void getFromCacheWhenCacheInvalidateAll() {
        ...
        assertEquals(initial, cached);
        service.value = "2";
        service.evictAll().block(Duration.ofMinutes(1)).orElseThrow();
