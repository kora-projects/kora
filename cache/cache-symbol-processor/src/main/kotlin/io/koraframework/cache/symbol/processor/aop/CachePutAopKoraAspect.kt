...
class CachePutAopKoraAspect(private val resolver: Resolver) : AbstractAopCacheAspect() {
    ...
    override fun apply(ksFunction: KSFunctionDeclaration, superCall: String, aspectContext: KoraAspect.AspectContext): KoraAspect.ApplyResult {
        if (ksFunction.isFuture()) {
            throw ProcessingErrorException("@${ANNOTATION_CACHE_PUT.simpleName} can't be applied for types assignable from ${Future::class.java}", ksFunction)
        } else if (ksFunction.isCompletionStage()) {
            throw ProcessingErrorException("@${ANNOTATION_CACHE_PUT.simpleName} can't be applied for types assignable from ${CompletionStage::class.java}", ksFunction)
        } else if (ksFunction.isPublisher()) {
            throw ProcessingErrorException("@${ANNOTATION_CACHE_PUT.simpleName} can't be applied for types assignable from ${CommonClassNames.publisher}", ksFunction)
        } else if (ksFunction.isVoid()) {
            throw ProcessingErrorException("@${ANNOTATION_CACHE_PUT.simpleName} can't be applied for types assignable from ${Void::class.java}", ksFunction)
        }
