...
import io.koraframework.cache.symbol.processor.CacheOperationUtils.Companion.ANNOTATION_CACHE_PUT
import io.koraframework.cache.symbol.processor.CacheOperationUtils.Companion.ANNOTATION_CACHE_PUTS
...
class CachePutAopKoraAspect(private val resolver: Resolver) : AbstractAopCacheAspect() {
    ...
    override fun apply(ksFunction: KSFunctionDeclaration, superCall: String, aspectContext: KoraAspect.AspectContext): KoraAspect.ApplyResult {
        if (ksFunction.isFuture()) {
            throw ProcessingErrorException(unsupportedCacheReturnTypeError("@${ANNOTATION_CACHE_PUT.simpleName}", ksFunction, Future::class.java.toString()), ksFunction)
        } else if (ksFunction.isCompletionStage()) {
            throw ProcessingErrorException(unsupportedCacheReturnTypeError("@${ANNOTATION_CACHE_PUT.simpleName}", ksFunction, CompletionStage::class.java.toString()), ksFunction)
        } else if (ksFunction.isPublisher()) {
            throw ProcessingErrorException(unsupportedCacheReturnTypeError("@${ANNOTATION_CACHE_PUT.simpleName}", ksFunction, CommonClassNames.publisher.toString()), ksFunction)
        } else if (ksFunction.isVoid()) {
            throw ProcessingErrorException(unsupportedCacheReturnTypeError("@${ANNOTATION_CACHE_PUT.simpleName}", ksFunction, Void::class.java.toString()), ksFunction)
        }
