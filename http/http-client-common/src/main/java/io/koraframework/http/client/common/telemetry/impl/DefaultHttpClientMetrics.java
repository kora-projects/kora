...
public class DefaultHttpClientMetrics {
    ...
    public DefaultHttpClientMetrics(String clientConfigPath,
    ...
    protected Timer.Builder createMetricClientDuration(HttpClientRequest request, int statusCode, @Nullable Throwable throwable) {
        var staticTags = new ArrayList<Tag>(10 + this.config.tags().size());
        ...
        staticTags.add(Tag.of(DefaultHttpClientTelemetry.SYSTEM_CONFIG_PATH, clientConfigPath));
        staticTags.add(Tag.of(DefaultHttpClientTelemetry.SYSTEM_NAME_SIMPLE, clientSimpleName));
        staticTags.add(Tag.of(DefaultHttpClientTelemetry.SYSTEM_NAME_CANONICAL, clientCanonicalName));
