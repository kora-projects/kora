...
public class ThreadLocalByteBufferPool implements ByteBufferPool {
    ...
    private static final AtomicIntegerFieldUpdater<ThreadLocalByteBufferPool> currentQueueLengthUpdater = AtomicIntegerFieldUpdater.newUpdater(ThreadLocalByteBufferPool.class, "currentQueueLength");

    @SuppressWarnings({"unused", "FieldCanBeLocal"})
    private volatile int reclaimedThreadLocals = 0;
    private static final AtomicIntegerFieldUpdater<ThreadLocalByteBufferPool> reclaimedThreadLocalsUpdater = AtomicIntegerFieldUpdater.newUpdater(ThreadLocalByteBufferPool.class, "reclaimedThreadLocals");

    private volatile boolean closed;

    private final ThreadLocalByteBufferPool arrayBackedPool;
     ...
     */
    public ThreadLocalByteBufferPool(boolean direct, int bufferSize) {
     ...
     */
    public ThreadLocalByteBufferPool(boolean direct, int bufferSize, int maximumPoolSize, int threadLocalCacheSize, int leakDecetionPercent) {
        this.direct = direct;
        ...
        if(direct) {
            arrayBackedPool = new ThreadLocalByteBufferPool(false, bufferSize, maximumPoolSize, 0, leakDecetionPercent);
        } else {
     ...
     * @param threadLocalCacheSize The maximum number of buffers that can be stored in a thread local cache
     */
    public ThreadLocalByteBufferPool(boolean direct, int bufferSize, int maximumPoolSize, int threadLocalCacheSize) {
    ...
    private static class DefaultPooledBuffer implements PooledByteBuffer {
        ...
        private final ThreadLocalByteBufferPool pool;
        ...
        DefaultPooledBuffer(ThreadLocalByteBufferPool pool, ByteBuffer buffer, boolean detectLeaks) {
            this.pool = pool;
            this.buffer = buffer;
    ...
    private class ThreadLocalData {
        ...
        protected void finalize() throws Throwable {
            try {
                reclaimedThreadLocalsUpdater.incrementAndGet(ThreadLocalByteBufferPool.this);
                if (buffers != null) {
                    // Recycle them
                    ByteBuffer buffer;
