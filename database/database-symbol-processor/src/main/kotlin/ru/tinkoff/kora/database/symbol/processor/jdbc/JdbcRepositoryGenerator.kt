    ...
    private fun parseResultMapper(method: KSFunctionDeclaration, parameters: List<QueryParameter>, methodType: KSFunction): Mapper? {
        ...
        for (parameter in parameters) {
            if (parameter is QueryParameter.BatchParameter) {
                if (IntArray::class.asTypeName() == returnType.declaration.let { it as KSClassDeclaration }.toClassName()) {
                    return null
                } else if (LongArray::class.asTypeName() == returnType.declaration.let { it as KSClassDeclaration }.toClassName()) {
                    return null
                } else if (!isGeneratedKeys) {
