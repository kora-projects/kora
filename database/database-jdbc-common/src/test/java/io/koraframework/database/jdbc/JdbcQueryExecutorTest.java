...
class JdbcQueryExecutorTest {
    ...
    private static void withDb(PostgresParams params, Consumer<JdbcDataSource> consumer) throws SQLException {
        ...
        var dataSource = new HikariDataSource(HikariUtils.toHikariConfig(config));
        var db = new JdbcDataSource(dataSource, config, NoopDatabaseTelemetryFactory.INSTANCE, dataSource::close);
        db.init();
        try {
            consumer.accept(db);
        } finally {
