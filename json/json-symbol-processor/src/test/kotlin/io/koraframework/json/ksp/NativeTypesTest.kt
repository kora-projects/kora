...
class NativeTypesTest : AbstractJsonSymbolProcessorTest() {
    ...
    fun testNativeJavaNetReaderExtension() {
        compile0(
            ...
            """
            @KoraApp
            interface TestApp : io.koraframework.json.common.JsonCommonModule {
              @Root
              fun root(reader: io.koraframework.json.common.JsonReader<TestRecord>) = ""
            }
            """.trimIndent()
        )
    ...
    fun testNativeJavaNetWriterExtension() {
        compile0(
            ...
            interface TestApp : io.koraframework.json.common.JsonCommonModule {
              @Root
              fun root(writer: io.koraframework.json.common.JsonWriter<TestRecord>) = ""
            }
            """.trimIndent()
        )
        compileResult.assertFailure()
