...
public class PeriodConfigValueMapper implements ConfigValueMapper<Period> {

    @Nullable
    @Override
    public Period map(ConfigValue<?> value) {
        if (value.isNull()) {
            return null;
        }
    ...
    public static Period parsePeriod(ConfigValue<String> configValue) {
        ...
        var numberString = ConfigValueMapperUtils.unicodeTrim(s.substring(0, s.length() - unitString.length()));

        // this would be caught later anyway, but the error message
        // is more helpful if we check it here.
        if (numberString.isEmpty())
            throw new ConfigValueException(configValue.origin().config(), "No number in period value " + configValue.origin().path(), null);
        ...
        try {
            ...
            throw new ConfigValueException(configValue.origin().config(), "Could not parse period number %s in %s".formatted(numberString, configValue.origin().path()), null);
        }
    }
