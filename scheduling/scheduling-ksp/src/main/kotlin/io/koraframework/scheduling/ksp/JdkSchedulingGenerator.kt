...
class JdkSchedulingGenerator(val environment: SymbolProcessorEnvironment) {
    ...
    private val jdkSchedulingExecutor = ClassName("io.koraframework.scheduling.jdk", "JdkSchedulingExecutor")
    ...
    private fun configType(type: KSClassDeclaration, function: KSFunctionDeclaration, vararg params: ConfigParameter): TypeSpec {
        ...
        val configType = TypeSpec.interfaceBuilder(configClassName)
            ...
            .addFunction(FunSpec.builder("telemetry").returns(ClassName("io.koraframework.scheduling.common.telemetry", "JobTelemetryConfig")).addModifiers(KModifier.ABSTRACT).build())
        for (param in params) {
