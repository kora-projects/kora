...
public interface SchedulingModule {
    ...
    default SchedulingTelemetryConfig schedulingTelemetryConfig(Config config, ConfigValueExtractor<SchedulingTelemetryConfig> extractor) {
        return extractor.extract(config.get("scheduling.telemetry"));
    }

    @DefaultComponent
    default DefaultSchedulingTelemetryFactory defaultSchedulingTelemetryFactory(SchedulingTelemetryConfig config,
