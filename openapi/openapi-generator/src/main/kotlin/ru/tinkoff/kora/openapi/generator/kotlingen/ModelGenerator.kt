...
class ModelGenerator : AbstractKotlinGenerator<ModelsMap>() {
    ...
    private fun buildRecord(ctx: ModelsMap, model: CodegenModel): TypeSpec {
        ...
        val parentFields = mutableSetOf<String>()
        for (entry in models.entries) {
            ...
            m.composedSchemas?.oneOf?.let {
                ...
                if (isSuper) {
                    parentFields.addAll(m.allVars.map { it.name })
                }
            }
        ...
        if (params.codegenMode.isClient()) {
            b.addType(
                TypeSpec.classBuilder("StringParameterConverter")
                    ...
                    .addProperty(PropertySpec.builder("delegate", Classes.enumStringParameterConverter.asKt().parameterizedBy(enumClassName))
            ...
            b.addType(
                TypeSpec.classBuilder("StringParameterReader")
                    ...
                    .addProperty(PropertySpec.builder("delegate", Classes.enumStringParameterReader.asKt().parameterizedBy(enumClassName))
