...
public abstract class AbstractGenerator<C, R> {
    ...
    protected String serverOperationAuthName(CodegenOperation op) {
        var operationAuthMethods = new TreeSet<String>();
        for (var authMethod : op.authMethods) {
            var authName = serverAuthName(authMethod);
            operationAuthMethods.add(authName);
        }
        return String.join("With", operationAuthMethods);
    }

    protected String serverAuthName(CodegenSecurity authMethod) {
        if (authMethod.isApiKey || authMethod.isBasic || authMethod.isBasicBearer) {
            return capitalize(toVarName(authMethod.name));
        }
        if (authMethod.isOAuth) {
            if (authMethod.scopes == null || authMethod.scopes.isEmpty()) {
                return capitalize(toVarName(authMethod.name) + "NoScopes");
            }
            var scopes = authMethod.scopes.stream()
                .map(it -> capitalize(toVarName(it.get("scope").toString())))
                .sorted()
                .collect(Collectors.joining("With"));
            return capitalize(toVarName(authMethod.name) + "With" + scopes);
        }
        throw new IllegalStateException();
    }


    protected record ServerAuthMethodGroup(String name, List<CodegenSecurity> methods) {}

    protected List<ServerAuthMethodGroup> collectServerAuthMethodGroups() {
        var authMethods = new ArrayList<ServerAuthMethodGroup>();
        for (var opMap : operationsByClassName.values()) {
            for (var op : opMap.getOperations().getOperation()) {
                if (op.hasAuthMethods) {
                    ...
                    var authInterceptorTag = serverOperationAuthName(op);
                    if (authMethods.stream().noneMatch(a -> a.name.equals(authInterceptorTag))) {
                        authMethods.add(new ServerAuthMethodGroup(authInterceptorTag, security));
                    } else {
                        // some check maybe?
                    }
                }
    ...
    protected List<String> collectServerAuthTags() {
