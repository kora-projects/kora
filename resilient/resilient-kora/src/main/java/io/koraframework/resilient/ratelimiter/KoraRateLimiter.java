 ...
 *   <li>If the stored period differs from the current one, reset the counter to 1.</li>
 *   <li>If the stored count is below {@code limitForPeriod}, increment and allow.</li>
 *   <li>Otherwise, reject the acquire.</li>
 * </ol>
 */
final class KoraRateLimiter implements RateLimiter {
    ...
    KoraRateLimiter(String name, RateLimiterConfig.NamedConfig config, RateLimiterTelemetry telemetry) {
        this.name = name;
        this.config = config;
    ...
    public boolean tryAcquire() {
        ...
        try {
            if (Boolean.FALSE.equals(config.enabled())) {
                observation.recordAcquire(true);
                return true;
            }
            ...
            while (true) {
                ...
                if (storedPeriodMark != currentPeriodMark) {
                    ...
                    observation.recordAcquire(false);
                    return false;
                }
